...
def process_institution(slug, props):
    ...
    #    )
    #    if df.empty:
    #        df = pd.read_csv(props["input"], dtype=str).head(1)
    #
    #    print(f"• DEBUG MODE: sampled {len(df)} / {n_before} rows")

    df["author_id_short"] = df["author_id"].str.rstrip("/").str.split("/").str[-1]
    df["inst_id_short"]   = df["institution_id"].str.rstrip("/").str.split("/").str[-1]
